...
escolar_peso_idade <- function(ano){
  ...
  filled_form <- set_values(filled_form, "nu_idade_inicio" = "5")
  filled_form <- set_values(filled_form, "nu_idade_fim" = "10")
  filled_form <- set_values(filled_form, "nu_indice_cri" = "1")
  ...
  z <- z[-c(1),c(4, 5, 6, 8, 10, 12, 14)] 
  
  # RENOMEIA AS VARIÁVEIS
  z <- z %>% 
    rename(codigo_municipio = 1,
           municipio = 2,
           peso_muito_baixo_para_a_idade = 3,
           peso_baixo_para_a_idade = 4,
           eutrofico = 5,
           peso_elevado_para_a_idade = 6,
           total = 7)
  ...
  file_name <- paste("C:/Users/Carlos Garcia Filho/OneDrive/Documentos/Unifor/Dados_Secundarios/Pesquisa_SISVAN/Dados/escolares_peso_idade",as.character(y),".csv", collapse = NULL) 
...
sapply(periodo, escolar_peso_idade)